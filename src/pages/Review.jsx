        ...
        if (user) {
          try {
            ...
            if (profiles.length > 0) {
              ...
              setRatingDisplayPreference(getRatingDisplayPreference(profiles[0]));
            }
          } catch (e) {
            console.error(e);
          }
          ...
          <div className="flex-1">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-4xl sm:text-5xl font-bold font-mono bg-gradient-to-r from-stone-300 to-slate-300 bg-clip-text text-transparent">
                {displayRating > 0 ? formattedDisplayRating.value : "—"}
              </span>
              {formattedDisplayRating.suffix && <span className="text-white/30 text-lg">{formattedDisplayRating.suffix}</span>}
            </div>
            {!useManualRating && (
          ...
          {!readOnly && (
            <>
              ...
              <div className="flex-1 max-w-full overflow-hidden">
                ...
                {useManualRating && (
                  <RatingInput value={manualRating} onChange={setManualRating} />
                )}
              </div>
            </>
          )}
